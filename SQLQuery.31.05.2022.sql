...
SELECT * FROM Notebooks WHERE EXISTs(SELECT * FROM Brands WHERE Brands.Name LIKE '%S%' AND Id=Notebooks.BrandsId)
...
SELECT Brands.Name, Count(*) 'Count' FROM Brands Join Notebooks On Notebooks. BrandId = Brands.Id Group By Brands.Name

--9) Her Branda Aid Nece dene Phone Varsa Brandin Adini Ve Yaninda Sayini Cixardan Query.
SELECT Brands.Name, Count(*) 'Count' FROM Brands Join Phones On Phones. BrandId = Brands.Id Group By Brands.Name
